 ...
 * @author root
 */
public class ActualizarPedidosTest {
    
    public ActualizarPedidosTest() {
    }
    ...
    public void ActualizarPedidoTest() throws Exception {
        ...
        resultado = ldp.get(0).getEstado() == EnumEstado.PREPARACION;
        assertTrue(resultado);
        
        try 
        {
           cp.seleccionarEstado(EnumEstado.PREPARACION);
           cp.actualizarPedido();  
           assertTrue(false);
        } catch (Exception ex) 
        {
        ...
        cp.seleccionarEstado(EnumEstado.ENVIADO);
        cp.actualizarPedido();  
        ldp = cp.listarPedidos("Bar17");
        resultado = ldp.get(0).getEstado() == EnumEstado.ENVIADO;
        assertTrue(resultado);
        ...
        cp.seleccionarEstado(EnumEstado.RECIBIDO);
        cp.actualizarPedido();
        ldp = cp.listarPedidos("Bar17");
        resultado = ldp.get(0).getEstado() == EnumEstado.RECIBIDO;            
